...
@php
    $check_user_roles = isset($admin) ? $admin->roles()->pluck('id')->toArray() : [];
@endphp
...
<div class="col-md-12">
    <div class="form-group">
        ...
        @foreach($roles as $role)
            <span class="ml-sm-3">{{ ucfirst($role->name) }}</span> <input value="{{ $role->id }}" {{ isset($admin) && in_array($role->id, $check_user_roles) ? "checked" : ''}} type="checkbox" name="type[]"  class="i-checks">
        @endforeach
...
<div class="col-md-12">
    ...
    @if(isset($admin))
        <img src="{{ @$admin->image->url }}" class="thumbnail" width="100px" height="80px">
        <br>
        <br>
    @endif
</div>
